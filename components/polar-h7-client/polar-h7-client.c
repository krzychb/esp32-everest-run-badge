...
static heart_rate_data heart_rate_sensor;

static const char remote_device_name[] = "Polar H7 EDAE7C17";
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
        ...
        if (srvc_id->id.uuid.len == ESP_UUID_LEN_16 && srvc_id->id.uuid.uuid.uuid16 == REMOTE_SERVICE_UUID) {
            ...
            ESP_LOGI(GATTC_TAG, "service found, UUID16: %x", srvc_id->id.uuid.uuid.uuid16);
        }
        break;
    }
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
        ...
        if (p_data->reg_for_notify.status != ESP_GATT_OK){
            ...
            ESP_LOGI(GATTC_TAG, "REG FOR NOTIFY done");
            uint16_t count = 0;
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ...
        heart_rate_sensor.heart_rate = (int) p_data->notify.value[1];
        if (heart_rate_sensor.data_retreived_cb) {
            heart_rate_sensor.data_retreived_cb((uint32_t*) &heart_rate_sensor);
        }
        //
        // Received HR from the server, now we can disconnect
        //
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            if (adv_name != NULL) {
                ESP_LOGI(GATTC_TAG, "found '%s', %d characters", adv_name, adv_name_len);
            } else {
                ESP_LOGE(GATTC_TAG, "Null string!");
            }
            if (adv_name != NULL) {
                if (strlen(remote_device_name) == adv_name_len && strncmp((char *)adv_name, remote_device_name, adv_name_len) == 0) {
                    ESP_LOGI(GATTC_TAG, "'%s' is what we have been searching for", remote_device_name);
                    if (connect == false) {
                        connect = true;
                        ESP_LOGI(GATTC_TAG, "connecting to the remote device '%s'", remote_device_name);
                        esp_ble_gap_stop_scanning();
                        esp_ble_gattc_open(gl_profile_tab[PROFILE_A_APP_ID].gattc_if, scan_result->scan_rst.bda, true);
                    } else {
                        ESP_LOGW(GATTC_TAG, "already connected to %s!", remote_device_name);
                    }
                }
            }
            break;
...
{
    heart_rate_sensor.data_retreived_cb = data_retreived_cb;
}
